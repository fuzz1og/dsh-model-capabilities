 ...
 * (SHA-256 of the session id): an `llm/stream` waterfall listener hands the
 * request's session token to a single `globalThis.fetch` wrapper through a
 * per-host FIFO ledger (the waterfall callback and the stream's single wire
 * fetch pair up one-to-one), which stamps matching outbound requests. The
 * token is stable across the turns of one session, differs across sessions,
 * and overrides a user-configured static `x-opencode-session` on the wire. The
...
function installSessionFetch(ledger, hosts) {
  ...
  const wrapped = async function fetch(input, init) {
    ...
    if (host === null || !hostMatches(host, hosts)) return original(input, init);
    const token = ledger.take(host);
    if (token === undefined) return original(input, init);
    try {
      const headers = new Headers(init === undefined || init === null ? undefined : init.headers);
      headers.set(AFFINITY_HEADER, token);
    ...
    try {
      if (!affinityDisabled && options !== null && typeof options === 'object' && typeof options.provider === 'string') {
        ...
        if (affinityForProfile(profile, hosts, false) === 'per-session' && typeof options.sessionId === 'string' && options.sessionId !== '') {
          const host = baseURLHost(profile.baseURL);
          if (host !== null) ledger.enqueue(host, affinityToken(options.sessionId));
        }
      }
    } catch {
