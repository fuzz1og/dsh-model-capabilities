    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && url.pathname === '/model-capabilities') {
          ...
          // Per-model edits need a stored models list. Route-level fields
          // (headers, compat, reasoning, defaultInput) do NOT: a catalog route
          // declares no models but still owns all four, and the standalone
          // settings page edits them there. So the models list is required only
          // when the payload actually carries per-model edits — previously this
          // refused the whole request, which made a catalog route's headers
          // uneditable.
          ...
          const nextModels = Array.isArray(profile.models) ? profile.models.map((model) => {
          ...
          const ops = [];
          // Only rewrite the models list when this payload actually edits a
          // model. A route-level-only save (headers alone) must not materialize
          // a `models` key on a catalog route, which would replace the served
          // catalog with an empty list.
          if (Array.isArray(profile.models) && byId.size > 0) {
            ops.push({ op: 'set', path: ['providers', provider, 'models'], value: nextModels });
          }
