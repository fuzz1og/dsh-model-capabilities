 ...
 *        schema-validated).
 *
 * Headers carry the opencode Go session-affinity requirement: the official
 * docs (https://opencode.ai/docs/go/) ask tools to include `x-opencode-session`
 * so the gateway can route per session and optimize prompt caching. The
 ...
 * headers may not override it. Values are static per provider (install-level
 * affinity); per-session rotation would need wire-level request wrapping,
 * which is outside this plugin's settings-UI scope.
 *
