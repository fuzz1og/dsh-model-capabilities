...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.mc-headerName{flex:1 1 40%;min-width:80px}',
      '.mc-headerValue{flex:2 1 50%;min-width:90px}',
      ...
      '.mc-modelSummary{display:inline-flex;gap:6px;flex-wrap:nowrap;overflow:hidden;min-width:0;justify-content:flex-end}',
      ...
      /* The editor now lives only in the page's right pane, which measures
         ~250-300px on a 1280px window — the inline card it replaced was as wide
         as the whole settings panel. Every minimum below must fit that pane:
         the compat grid's old 310px column overflowed it by 54px (measured). */
      '.mc-compatGrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(228px,1fr));gap:6px 14px}',
      '.mc-compatGrid .mc-label{width:126px}',
      ...
      '.mcp-grid{display:grid;grid-template-columns:210px minmax(0,1fr);gap:0;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;overflow:hidden;background:var(--dsw-alias-bg-layer-1);min-height:420px}',
      ...
      '.mcp-pane{padding:16px 18px;overflow:auto;max-height:70vh;min-width:0}',
    ...
    function apply(ctx) {
      ...
      // One surface only: the standalone page below. The inline
