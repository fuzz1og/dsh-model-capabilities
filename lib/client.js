 ...
 *     (names lowercased; user-agent reserved). opencode Go session affinity is
 *     NOT edited here — the Host face injects a per-DSH-session
 *     `x-opencode-session` at the wire layer automatically for matching hosts
 *
        ...
        e(DisclosureRow, {
          ...
          children: e('div', { className: 'mc-modelBody' },
            snap.sessionAffinity === 'per-session' ? e('div', { className: 'mc-note' },
              'opencode 官方要求（opencode.ai/docs/go）：Go 套餐请求应携带 x-opencode-session（按会话亲和路由、优化 prompt caching）。本网关已匹配，插件在 wire 层自动注入——每个 DSH 会话一个独立稳定标识（SHA-256 摘要，跨轮次不变），无需手动配置；手动添加同名请求头会在 wire 上被会话注入覆盖。DSH 亦已发送真实归属 User-Agent，无需伪装 opencode 客户端。')
              : opencodeDetected ? e('div', { className: 'mc-note' },
                'opencode 官方要求（opencode.ai/docs/go）：Go 套餐请求应携带 x-opencode-session。该提供方未命中默认网关匹配（opencode.ai），可手动添加固定值；或在 profile 层按行 config 扩展 hosts 后自动按会话注入。DSH 已发送真实归属 User-Agent，无需伪装 opencode 客户端。')
              : null,
            ...
            e('div', { className: 'mc-row' },
              ...
              e('span', { className: 'mc-note' }, '按需手动添加；名称写入时统一为小写；user-agent 不可设置（DSH 归属头占用）'))),
        })));
