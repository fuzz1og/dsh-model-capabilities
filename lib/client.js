 ...
 *   - provider request headers: x-opencode-session (the opencode Go
 *     session-affinity header required by https://opencode.ai/docs/go/) and
 *     arbitrary custom headers; names merge case-insensitively, user-agent is
 *     reserved by the harness attribution and rejected both here and upstream
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** A short opaque stable-affinity token, e.g. `dsh-k3j9x2m4qf7n`. */
    function randomSessionToken(length = 12) {
      const alphabet = 'abcdefghijklmnopqrstuvwxyz0123456789';
      const bytes = new Uint8Array(length);
      if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') crypto.getRandomValues(bytes);
      else for (let at = 0; at < length; at += 1) bytes[at] = Math.floor(Math.random() * 256);
      let token = 'dsh-';
      for (let at = 0; at < length; at += 1) token += alphabet[bytes[at] % alphabet.length];
      return token;
    }
    function headerRowsOf(dict) {
      const rows = Object.entries(dict ?? {}).map(([name, value]) => ({ name, value: typeof value === 'string' ? value : '' }));
      rows.sort((a, b) => (a.name === SESSION_HEADER ? -1 : b.name === SESSION_HEADER ? 1 : a.name < b.name ? -1 : 1));
      return rows;
    }
    ...
    function ModelCapabilities(props) {
      ...
      const setSessionHeader = (present) => patchHeaders((rows) => {
        const kept = rows.filter((row) => row.name.trim().toLowerCase() !== SESSION_HEADER);
        return present ? [{ name: SESSION_HEADER, value: randomSessionToken() }, ...kept] : kept;
      });
      const setSessionHeaderValue = (value) => patchHeaders((rows) => rows.map((row) => row.name.trim().toLowerCase() === SESSION_HEADER ? { ...row, value } : row));
      ...
      const apply = async () => {
        ...
        try {
          ...
          if (json && json.ok === true) {
            ...
            setStatus({ state: 'error', text: 'conflict — 设置已被其他编辑修改（revision 过期），刷新后重试', error: true });
          } else {
      ...
      const sessionRow = snap.headerRows.find((row) => row.name.trim().toLowerCase() === SESSION_HEADER);
      const genericEntries = snap.headerRows.map((row, index) => ({ row, index })).filter(({ row }) => row.name.trim().toLowerCase() !== SESSION_HEADER);
      ...
      parts.push(e('div', { key: 'headers' },
        e(DisclosureRow, {
          title: '请求头 · 会话亲和 (x-opencode-session)',
          open: openHeaders,
          expandable: true,
          expandOnRowClick: true,
          keepContentWhenOpen: true,
          ...
          collapsedContent: e('span', { className: 'mc-note' },
            sessionRow === undefined
              ? (opencodeDetected ? '未设置 — opencode Go 官方要求携带该头' : '未设置')
              : `x-opencode-session: ${sessionRow.value.trim() === '' ? '（空）' : sessionRow.value}`,
            genericEntries.length > 0 ? ` · 自定义 ${genericEntries.length}` : null),
          children: e('div', { className: 'mc-modelBody' },
            opencodeDetected ? e('div', { className: 'mc-note' },
              'opencode 官方要求（opencode.ai/docs/go）：使用 Go 套餐的工具应携带 x-opencode-session（按会话亲和路由、优化 prompt caching），否则账号可能被标记。DSH 已对每个提供方请求发送真实归属 User-Agent（deepseek-harness/版本），身份要求已满足，无需伪装 opencode 客户端。') : null,
            e('div', { className: 'mc-row' },
              e('span', { className: 'mc-label' }, '会话亲和'),
              e(Segmented, {
                value: sessionRow === undefined ? 'unset' : 'fixed',
                options: [{ value: 'unset', label: '未设置' }, { value: 'fixed', label: '固定值' }],
                onChange: (value) => setSessionHeader(value === 'fixed'),
              }),
              sessionRow === undefined ? null : e(Input, {
                className: 'mc-effortInput',
                value: sessionRow.value,
                placeholder: '稳定的不透明会话标识',
                onChange: (event) => setSessionHeaderValue(event.target.value),
              }),
              sessionRow === undefined ? null : e(Button, { variant: 'outline', size: 'sm', onClick: () => setSessionHeaderValue(randomSessionToken()) }, '生成新值')),
            e('div', { className: 'mc-note' },
              '值为该提供方固定的安装级亲和标识（同一 DSH 的所有会话共用；每会话轮换需要 wire 层注入类插件或上游适配器支持）。写入经 pi-ai schema 校验（Fetch 头合法性），拒绝会原样显示。'),
            e('div', { className: 'mc-section' },
              e('div', { className: 'mc-sectionTitle' }, '自定义请求头'),
              genericEntries.map(({ row, index }) => e('div', { className: 'mc-row', key: `hdr-${index}` },
                ...
                e(Input, { className: 'mc-headerValue', value: row.value, placeholder: '值', onChange: (event) => editHeaderRow(index, { value: event.target.value }) }),
              ...
              e('div', { className: 'mc-row' },
                ...
                e('span', { className: 'mc-note' }, '名称写入时统一为小写；user-agent 不可设置（DSH 归属头占用）')))),
        })));
