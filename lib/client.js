    ...
    function ModelCapabilitiesPage() {
      ...
      const loadIndex = async () => {
        try {
          ...
          setIndex(rows);
          // Keep the current selection when it still exists, else take the first.
          setSelected((current) => rows.some((row) => row.provider === current) ? current : rows[0]?.provider);
        } catch (error) {
          setStatus({ state: 'error', text: String(error), error: true });
          setIndex([]);
        }
      ...
      const nav = e('div', { className: 'mcp-nav' },
        ...
        rows.map((row) => e('div', { key: row.provider },
          e('div', {
